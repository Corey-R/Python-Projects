...
def center_window(self, w, h): # pass in the tkinter frame (master) reference and the w and h
    ...
    screen_width = self.master.winfo_screenwidth() # <-- get's the user's screen width
    screen_height = self.master.winfo_screenheight() # <-- method get's the user's screen height
...
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Okay to exit application?"):
        # This closes the app
        self.master.destroy()
        os._exit(0) # os method used to close all widgets and frees up space for user's device (preventing memory leaks)
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): # enforce the user to provide both names
        ...
        with conn:
            ...
            chkName = count
            if chkName == 0: # if this is 0 than there is no existence of the fullname and we can add new data
                ...
                onClear(self) # call this function to clear all of the texboxes
            else:
                messagebox.showerror("Name Error", "'{}' already exists in the database! Please choose a different name.".format(var_fullname))
                onClear(self) # call this function to clear all of the textboxes
        conn.commit()
        conn.close()
    else:
